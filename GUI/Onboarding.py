...
def start_onboarding_ui(speaking_event, on_ready) -> None:
    """Run onboarding UI on the main thread (single continuous GUI).

    Shows the Jarvis circle on the left and the setup panel on the right.
    After saving, collapses the panel and invokes on_ready() to continue startup.
    """
    root = tk.Tk()
    root.title("Jarvis")
    root.geometry("740x360")
    ...
    fg = "#23d160"; bg = "#0f0f0f"; border_w = 10
    R = (canvas_size // 2) - 12; mouth_max_inset = 30
    phase = {"v": 0.0}
    ...
    ttk.Button(panel, text="Save & Continue", command=save_and_continue).grid(row=2, column=2, sticky="e", pady=(8, 0))
